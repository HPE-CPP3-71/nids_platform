...
NOTE: The classifier now only routes Discover packets (type 1)
to this extractor. Request packets from spoofing traffic are
excluded at the classifier level to prevent false positives.
As a safety net, request_count is also zeroed if no Discovers
are seen in the window.
"""
...
class DHCPStarvationFeatureExtractor(BaseFeatureExtractor):
    ...
    Processes a WindowBatch of DHCP Discover packets and produces
    a FeatureVector with 6 features matching the training
    notebook's process_pcap() function.

    Since the classifier now exclusively routes Discover packets
    here, the window will only ever contain type-1 packets.
    request_count is kept for completeness but will typically
    be 0 unless mixed traffic reaches this extractor.
    """
    ...
    def extract(self, batch: WindowBatch) -> FeatureVector:
        ...
        for record in batch.packets:
            ...
            if dtype == 1:  # DHCP Discover
                ...
                # but count them only if Discovers are also present.
                # This prevents spoofing Requests from inflating
                # request_count when discover_count is 0.
                request_count += 1

        # Safety net: if no Discovers seen, zero out request_count.
        # Requests without Discovers = spillover from spoofing traffic,
        # not a starvation signal.
        if discover_count == 0:
            request_count = 0
